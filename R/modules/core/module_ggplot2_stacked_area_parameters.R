ggplot2_parameters_stacked_area_UI <- function(id){
  fluidPage(
    fluidRow(
      align = "center",
      column(width = 3, bgc_column_select(id, "x_axis", "Set x axis variable")),
      column(width = 3, bgc_column_select(id, "y_axis", "Set y axis variable (numeric)")),
      column(width = 3, bgc_column_select(id, "fill_variable", "Set fill / group variable")),
      column(width = 3,
             prettyRadioButtons(NS(id, "stack_position"),
                                label = "Stack Type",
                                choiceNames = c("Stack","Fill (100%)","Identity"),
                                choiceValues = c("stack","fill","identity"),
                                icon = icon("check"),
                                animation = "tada",
                                inline = TRUE))
    ),
    fluidRow(
      align = "center",
      column(width = 3, sliderInput(NS(id, "alpha"), "Fill Alpha", value = 0.7, min = 0, max = 1, step = 0.05)),
      column(width = 3, sliderInput(NS(id, "line_size"), "Line Size", value = 0.5, min = 0, max = 3, step = 0.1)),
      column(width = 3, colourInput(NS(id, "line_color"), "Line Color", value = "#FFFFFF")),

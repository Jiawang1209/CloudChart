ggplot2_parameters_rigeline_UI <- function(id){
  fluidPage(
    fluidRow(
      align = "center",
      # Set x Variable
      ...
      # Set alpha
      column(width = 3, sliderInput(NS(id, "alpha"), "Density Alpha", value = 0.4, min = 0, max = 1)),
    ),
    tags$hr(),
    fluidRow(
      align = "center",
      # Set line color
      column(width = 3, colourInput(NS(id, "line_color"), "Set Line Color", value = "#000000")),
      # Set add quantile lines
      column(width = 3,
             pickerInput(NS(id, "quantile_lines"),
                         label = "add Quantile Type",
                         choices = c("Yes", "No"),
    ...
    fluidRow(
      align = "center",
      # Set line width
      column(width = 3, sliderInput(NS(id, "line_width"), "Set Line Width", value = 0.5, min = 0, max = 5, step = 0.5)),
      # Set rug height
      column(width = 3, sliderInput(NS(id, "rug_height"), "Set bar height", value = 3, min = 0, max = 10)),
      # Set Scale
      column(width = 3, sliderInput(NS(id, "scale_data"), "Set Scale value", value = 1, min = 0, max = 5, step = 0.25))
    ),

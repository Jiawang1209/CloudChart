ggplot2_dotplot_Server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    observeEvent(df(), {
      ...
      sync_column_choices(
        session,
        df(),
        c("x_axis", "y_axis", "fill_variable", "shape_variable"),
        selected_map = list(
          x_axis = bgc_pick_column(input$x_axis, discrete_cols[1]),
          y_axis = bgc_pick_column(input$y_axis, numeric_cols[1]),
          fill_variable = bgc_pick_column(input$fill_variable, discrete_cols[2], numeric_cols[2]),
          shape_variable = bgc_pick_column(input$shape_variable, discrete_cols[3], discrete_cols[2])
        )
      )
    }, ignoreNULL = FALSE)

    dot_mapping <- reactive({
      req(input$x_axis != "", input$y_axis != "")
      validate_required_columns(df(), c(input$x_axis, input$y_axis, input$fill_variable, input$shape_variable))
      ...
      if (input$shape_variable != "") {
        mapping$shape <- rlang::expr(factor(.data[[!!input$shape_variable]]))
      }
    ...
    bind_plot_outputs(output, input, dot_plot_reac, vals, "Dot_Plot")

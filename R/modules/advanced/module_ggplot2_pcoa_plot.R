ggplot2_pcoa_Server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    PCoA_table.pca <- reactive({
      as.data.frame(PCoA_out()$vectors[,1:2])
    })
    
    PCoA_table.pca_out <- reactive({
      tmp <- PCoA_table.pca()
      ...
      if (input$add_ellipse == "No") {
        if (input$shape_variable == "") {
          p <- ggplot(PCoA_table.pca_out(),
            ...
            labs(x = paste('PCoA1 (', paste0(round(as.numeric(PCoA_out()$values[1,3]), digits = 2) * 100, '%'), ')', sep = ''),
                 y = paste('PCoA2 (', paste0(round(as.numeric(PCoA_out()$values[2,3]), digits = 2) * 100, '%'), ')', sep = ''),
                 title = input$plot_title,
                 subtitle = input$plot_subtitle
            ) + 
            get(Plot_theme())() + 
          ...
          p <- ggplot(PCoA_table.pca_out(),
            ...
            labs(x = paste('PCoA1 (', paste0(round(as.numeric(PCoA_out()$values[1,3]), digits = 2), '%'), ')', sep = ''),
                 y = paste('PCoA2 (', paste0(round(as.numeric(PCoA_out()$values[2,3]), digits = 2), '%'), ')', sep = ''),
                 title = input$plot_title,
                 subtitle = input$plot_subtitle
            ) + 
            get(Plot_theme())() + 
    ...
    output$Download <- downloadHandler(
      filename = function(){paste0("PCoA_Plot_", Sys.Date(),".pdf")},

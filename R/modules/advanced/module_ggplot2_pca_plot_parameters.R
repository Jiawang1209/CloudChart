ggplot2_parameters_pca_UI <- function(id){
  fluidPage(
    fluidRow(
      align = "center",
      # Set x Axis Variable
      column(width = 2, bgc_column_select(id, "x_axis", "Set x axis variable", selected = "Dim.1")),
      # Set y Axis Variable
      column(width = 2, bgc_column_select(id, "y_axis", "Set y axis variable", selected = "Dim.2")),
      # Set fill Variable
      column(width = 2, bgc_column_select(id, "fill_variable", "Set fill variable")),
      # Set color Variable
      column(width = 2, bgc_column_select(id, "color_variable", "Set color variable")),
      # # Set shape Variable,
    ...
    fluidRow(
      align = "center",
      column(width = 12, bgc_multi_column_select(id, "metadata_columns", "Metadata / Group Columns"))
    ),
    tags$hr(),
    fluidRow(
      align = "center",
      # Set Point Size
      column(width = 3, sliderInput(NS(id, "point_size"), "Point Size", value = 8, min = 0, max = 20, step = 0.5)),
      # Set Point alpha
      column(width = 3, sliderInput(NS(id, "point_alpha"), "Point Alpha", value = 0.5, min = 0, max = 1, step = 0.1)),
      # Set Add ellipse
      column(width = 3, 
